...
// 	$i=1;
	...
	*/
	...
	if(!$all){$db->query("INSERT INTO bron_time(id_room,date,seans_1,seans_2,seans_3,seans_4,seans_5,seans_6,seans_7,seans_8,seans_9) VALUES('1','".$date."','0','0','0','0','0','0','0','0','0'),('2','".$date."','0','0','0','0','0','0','0','0','0'),('3','".$date."','0','0','0','0','0','0','0','0','0')");}
	$timtm=0;
	if(date("w",$timtm)==0){$W='Сентябрь';}
	if(date("w",$timtm)==1){$W='Октябрь';}
	if(date("w",$timtm)==2){$W='Ноябрь';}
	if(date("w",$timtm)==3){$W='Декабрь';}
	if(date("w",$timtm)==4){$W='Июнь';}
	if(date("w",$timtm)==5){$W='Июль';}
	if(date("w",$timtm)==6){$W='Август';}
	if(date("w",$timtm)==7){$W='Сентябрь';}
	if(date("w",$timtm)==8){$W='Октябрь';}
	if(date("d.m.Y")==$date){$dop_cl='fiol';}else{$dop_cl='';}
	$result='<tr><td style="width: 35%;height:75px;"><div class="fiol" style="margin-top: -10px;">'.date("d.m.Y").'<br><span style="font-size:12px;">Сегодня</span></td>
	<td><span onClick="days(\''.date("d.m.Y").'\');" class="days_b '.$dop_cl.'">'.date("d").'</span><div class="top_w">'.$W.'</div></td>';
$timtm=86400;
	...
	for($i;$i<7;$i++){
	if(date("d.m.Y",time()+$timtm)==$date){$dop_cl='fiol';}else{$dop_cl='';}
	$result.='<td><span onClick="days(\''.date("d.m.Y",time()+$timtm).'\');" class="days_b '.$dop_cl.'">'.date("m",time()+$timtm).'</span>';
	if(date("w",$timtm)==0){$W='Сентябрь';}
	if(date("w",$timtm)==1){$W='Октябрь';}
	if(date("w",$timtm)==2){$W='Ноябрь';}
	if(date("w",$timtm)==3){$W='Декабрь';}
...
if($rr==1){$da='Стандарт';}
if($rr==2){$da='Люкс';}
if($rr==3){$da='Люкс-VIP';}$rr++;
$result.='<tr><td style="text-align:left;"><a href="/'.$da.'/" style="text-decoration:none;color:#fff;"><div class="fiol" style="cursor:pointer;">'.$vals['name_room'].'<br><span style="font-size:13px;">2-4 человека, от 24 часов</span></div></a></td>
...
if($room['seans_8']==1){$seans_8='background:'.$color4ik.';';}
if($room['seans_9']==1){$seans_9='background:'.$color4ik.';';}
$result.='<tr><td style="text-align:left;"><div class="fiol">'.$vals['name_room'].'<br><span style="font-size:13px;">2-4 человека, от 24 часов</span></div></td>
